...
def detect_search_intent(keyword):
    """
    Classify a Hebrew keyword into one of three search intents:
      - 'informational': user wants an explanation or answer (מה, איך, האם...)
      - 'commercial':    user is evaluating a decision (price, location, comparison)
      - 'navigational':  user already knows what they want, looking for the specific course
    """
    kw = keyword

    informational_signals = [
        'מה ', 'מה ה', 'מהו', 'מהי', 'איך ', 'כיצד', 'האם ', 'למה ', 'מדוע',
        'מתי ', 'מדריך ל', 'הסבר', 'הגדרה', 'הבדל בין',
    ]
    commercial_signals = [
        'מחיר', 'עלות', 'כמה עולה', 'תעריף', 'כדאי', 'השוואה',
        'הכי טוב', 'מומלץ', 'איפה ל', 'ירושלים', 'באר שבע',
        'גבעת חביבה', 'צפון', 'דרום', 'מרכז',
    ]
...
def build_blog_prompt(topic_data, config, site_context=None):
    ...
    # SERP snippets — what Google currently shows as the best answer for this query
    snippets_raw = topic_data.get("serp_snippets", [])
    if snippets_raw:
        serp_snippets_block = "\n".join(
            f"מיקום {s['position']}: {s['title']}\n  {s['snippet']}"
            for s in snippets_raw
        )
    else:
        serp_snippets_block = "לא נמצאו קטעי SERP"

    # Competitor content previews — first ~800 chars of each top competitor's page
    previews_raw = comp.get("content_previews", [])
    if previews_raw:
        content_previews_block = "\n\n".join(
            f"[{p['url']}]\n{p['preview']}"
            for p in previews_raw
        )
    else:
        content_previews_block = "לא נמצאו תצוגות מקדימות"

    # Intent detection — drives structure and direct-answer block
    intent = detect_search_intent(target_kw)

    if intent == 'informational':
        structure_block = f"""=== מבנה לפי כוונת הגולש: מידעני ===
הגולש שאל שאלה ישירה ומצפה להסבר. סדר מומלץ:
1. פתיחה: תשובה קצרה לשאלה הראשית (1-2 משפטים)
2. === תשובה ישירה === — חובה לכלול
3. H2 לכל תת-שאלה שגולש ישאל בהמשך (ארגן H2s לפי שאלות, לא לפי פרקים כלליים)
4. דוגמאות, מספרים ספציפיים, מקרים מהשטח
5. FAQ מורחב בתחתית (5+ שאלות)"""
        direct_answer_req = """4. בלוק תשובה ישירה (חובה — שאילתה מידענית): מיד לאחר פסקת הפתיחה:
   === תשובה ישירה (לציטוט AI) ===
   [תשובה ישירה בדיוק 40-60 מילים — משפט שלם ועצמאי, עונה על השאילתה ללא הקדמה]"""
        direct_answer_format = "\n=== תשובה ישירה (לציטוט AI) ===\n[40-60 מילים שעונות ישירות על השאילתה הראשית]\n"

    elif intent == 'commercial':
        structure_block = f"""=== מבנה לפי כוונת הגולש: מסחרי ===
הגולש שוקל החלטה — ענה ישירות על השאלה המעשית. סדר מומלץ:
1. פתיחה: ענה ישירות (עלות / מיקום / מה כלול) — לא סיפור רקע
2. H2: "מה כלול בקורס?" — ערך ספציפי שהלומד מקבל
3. H2: "כמה עולה הקורס?" — טווח מחירים, מסלולי תשלום
4. H2: "איפה ובאיזה פורמט?" — קמפוסים, שעות, אונליין/פרונטלי
5. H2: "מה מקבלים בסוף?" — תעודה, הזדמנויות
6. קריאה לפעולה ברורה
7. FAQ קצר (2-3 שאלות בלבד)"""
        direct_answer_req = "4. תשובה ישירה: כלול בלוק תשובה ישירה רק אם ניתן לנסח תשובה מדויקת ב-40-60 מילים. אחרת — פתח ישירות עם המידע הרלוונטי."
        direct_answer_format = ""

    else:  # navigational — default for "קורס X" queries
        structure_block = f"""=== מבנה לפי כוונת הגולש: ניווטי ===
הגולש מחפש מידע ספציפי על הקורס עצמו. סדר מומלץ:
1. פתיחה: מה הקורס מקנה ולמי הוא מתאים (2-3 משפטים)
2. H2: "מה לומדים ב{target_kw}?" — תכנית לימודים ונושאים מרכזיים
3. H2: "למי הקורס מתאים?" — פרופיל, דרישות קדם, רמה
4. H2: "מה עושים אחרי {target_kw}?" — הזדמנויות, שכר, תעסוקה
5. H2: "איך נרשמים?" — מסלולים, קמפוסים, מועדים
6. FAQ: 3-4 שאלות מעשיות"""
        direct_answer_req = "4. תשובה ישירה: כלול בלוק תשובה ישירה רק אם הפוסט עונה על שאלה ישירה מפורשת. בפוסטים ניווטיים — לא חובה."
        direct_answer_format = ""
    ...
    # COURSE_META block — only for navigational queries (course-type pages)
    course_meta_format = ""
    course_meta_req = ""
    if intent == "navigational":
        course_meta_format = (
            "\nCOURSE_META:\n"
            "DURATION: [משך הקורס בפועל — לדוגמה: 80 שעות / 3 חודשים. אם לא ידוע — לא צוין]\n"
            "CERTIFICATE: [שם התעודה שמקבלים — לדוגמה: תעודת הסמכה משרד התחבורה. אם לא ידוע — לא צוין]\n"
            "PREREQUISITES: [דרישות קדם — לדוגמה: רישיון B בתוקף / אין דרישות קדם]\n"
            "TEACHES: [3-5 מיומנויות מרכזיות מופרדות בפסיקים — לדוגמה: נהיגה בטוחה, תקנות תעבורה, עזרה ראשונה]"
        )
        course_meta_req = "\n14. COURSE_META: מלא את פרטי הקורס המדויקים לפי התוכן. השתמש רק במידע שמופיע בפוסט — אל תמציא."
...
דוגמאות לזוויות ייחודיות: ניסיון מהשטח עם לקוחות, השוואה בין מוצרים/שירותים ספציפיים מהקטלוג שלנו, טעויות נפוצות שרואים בפועל, מדריך בחירה לפי פרופיל הגולש, רמת ניסיון, או מצב ספציפי — בהתבסס על המוצרים/שירותים שלנו.
...
Information Gain (מדיניות Google Helpful Content — חובה): הפוסט חייב לכלול לפחות סקציה אחת עם תובנה מקורית שלא מופיעה אצל המתחרים — נתון שנמדד בפועל, תוצאה ספציפית מניסיון לקוחות, או המלצה מפורטת שנובעת מהכרות ייחודית עם המוצרים/שירותים של {site_name}. תוכן שגולש לא מוצא בשום מקום אחר.
נצל את קטלוג המוצרים/השירותים הייחודי של {site_name}: הצע המלצות ספציפיות עם שמות מוצרים ומותגים מהאתר שלנו — ערך שאתרי תוכן כלליים לא יכולים לספק.

=== אותות GEO (לציטוט על ידי AI) ===
תוכן שמצוטט על ידי Gemini, ChatGPT ו-Perplexity חייב להיות עשיר בעובדות שניתן לאמת.
כלול בגוף הפוסט:
• כלול מספרים ספציפיים רק אם הם מאומתים (כגון: משך קורס, דרישות רישיון, מחיר טווח). אל תמציא סטטיסטיקות — נתונים שנראים כממוצאים הם אות ספאם עבור Google ופוגעים בדירוג.
• לפחות 2 הצהרות דקלרטיביות חד-משמעיות שניתן לצטט ישירות, ללא סייגים: "X הוא...", "הכלל הוא ש-Y...", "הדרך הנכונה ל-Z היא..."
• ישויות מוגדרות (Named Entities) לעיגון סמכות: שמות מותגים/ארגונים ספציפיים לנושא הפוסט, מחקרים ומקורות שניתן לאמת, שנות פרסום
מטרה: AI יחלץ ויצטט עובדות ספציפיות ומדידות — לא הכללות. ככל שהמספר ספציפי יותר, כך גדל הסיכוי לציטוט.

=== דרישות ===
1. כותרת (H1): מקסימום 5 מילים — מילת המפתח הראשית בלבד.
   חוק קשיח: אסור לכלול "במכללת {site_name}", שם האתר, ביטויים שיווקיים כגון "הדרך שלך ל...", "הצעד הבא...", "קריירה מצליחה", "מדריך מלא".
   מותר: מילת המפתח הראשית + מילה-שתיים ספציפיות בלבד (למשל: סוג רישיון, מספר מסלול).
   דוגמה טובה: "{target_kw}"
   דוגמה רעה: "{target_kw} במכללת {site_name}: הצעד הבא בקריירה שלך" ← כותרת כזו הורגת את הדירוג.
   זכור: הכותרת הופכת ל-URL — כל מילה מיותרת היא מילה מיותרת ב-URL, וגוגל מעניש URL שיווקיים וארוכים.
...
8. קישורים פנימיים: שלב 3-5 קישורים פנימיים. השתמש אך ורק בדפים מרשימת "דפים באתר לקישור פנימי" שלמעלה — אסור להמציא URLs שאינם ברשימה. בחר דפים שקשורים תמטית לתוכן הפוסט. טקסט עוגן = שם הדף/קורס, לא "לחצו כאן".
...
12. סיכום מהיר (TL;DR): מיד לאחר פסקת הפתיחה — H3 "סיכום מהיר" עם 50-70 מילים עצמאיות. זהו הקטע שצוין על ידי AI. חובה בכל פוסט.
13. FAQ_SCHEMA: בסוף הפלט, העתק את כל שאלות ה-FAQ מהפוסט בפורמט FAQ_SCHEMA לעיל. כל תשובה: 40-60 מילים בדיוק.{course_meta_req}
...
SLUG: [slug — 3-4 מילות מפתח בלבד, ללא שם האתר, ללא ביטויים שיווקיים. דוגמה: kurs-menahel-avoda]
...
### סיכום מהיר
[50-70 מילים בדיוק — תשובה עצמאית ומלאה לשאילתה הראשית. ללא הקדמה. מיועדת לציטוט ישיר על ידי Google AI Overviews, Perplexity ו-ChatGPT. המשפט הראשון חייב לתת את התשובה המלאה.]

{direct_answer_format}
[גוף הפוסט בעברית עם כותרות markdown (## ל-H2, ### ל-H3) לפי המבנה המומלץ לעיל, וקישורים בפורמט [טקסט](URL)]
...
FAQ_SCHEMA:
Q: [שאלה 1 — משפט שאלה מלא ומדויק כפי שגולש יחפש]
A: [תשובה — בדיוק 40-60 מילים, עצמאית, ניתנת לציטוט ללא הקשר]
Q: [שאלה 2]
A: [תשובה]
Q: [שאלה 3]
A: [תשובה]
[המשך לכל שאלות ה-FAQ בפוסט — לפחות 4, מקסימום 8]{course_meta_format}
"""
...
הגוף מדורג טוב — אל תשנה את המבנה. שפר רק את תיאור המטא: הפוך אותו ספציפי, מספרי ומושך."""
    ...
    # Competitor content previews from competitor_summary
    rewrite_previews_raw = competitor_summary.get("content_previews", [])
    if rewrite_previews_raw:
        rewrite_content_previews_block = "\n\n".join(
            f"[{p['url']}]\n{p['preview']}"
            for p in rewrite_previews_raw
        )
    else:
        rewrite_content_previews_block = "לא נמצאו תצוגות מקדימות"

    context_block = ""
    if site_context:
        context_block = format_context_for_prompt(site_context, topic=post_data.get("title", ""))

    # COURSE_META block — only for Everest (course platform)
    course_meta_format = ""
    course_meta_req = ""
    if config.get("site", {}).get("name") == "Everest":
        course_meta_format = (
            "\nCOURSE_META:\n"
            "DURATION: [משך הקורס בפועל — לדוגמה: 80 שעות / 3 חודשים. אם לא ידוע — לא צוין]\n"
            "CERTIFICATE: [שם התעודה שמקבלים — לדוגמה: תעודת הסמכה משרד התחבורה. אם לא ידוע — לא צוין]\n"
        ...
        course_meta_req = "\n15. COURSE_META: מלא את פרטי הקורס המדויקים לפי התוכן. השתמש רק במידע שמופיע בפוסט — אל תמציא."
...
9. קישורים פנימיים: שלב 3-5 קישורים פנימיים. השתמש אך ורק בדפים מרשימת "דפים באתר לקישור פנימי" שלמעלה — אסור להמציא URLs שאינם ברשימה. בחר דפים שקשורים תמטית לתוכן הפוסט. טקסט עוגן = שם הדף/קורס, לא "לחצו כאן".
10. הזכר מוצרים ומותגים ספציפיים של {site_name} כשזה רלוונטי.
11. טון: מקצועי אבל חם, מדבר בסמכות על נושא הפוסט. לא מכירתי.
12. שפה: כתוב הכל בעברית.
13. סיכום מהיר (TL;DR): מיד לאחר פסקת הפתיחה — H3 "סיכום מהיר" עם 50-70 מילים עצמאיות. חובה בכל פוסט.
14. FAQ_SCHEMA: בסוף הפלט, העתק את כל שאלות ה-FAQ מהפוסט בפורמט FAQ_SCHEMA לעיל. כל תשובה: 40-60 מילים בדיוק.{course_meta_req}
...
### סיכום מהיר
[50-70 מילים בדיוק — תשובה עצמאית ומלאה לשאילתה הראשית. ללא הקדמה. מיועדת לציטוט ישיר על ידי Google AI Overviews, Perplexity ו-ChatGPT.]
...
10. סיים את הדף בקטע שאלות נפוצות (FAQ) עם בדיוק 3 שאלות ותשובות בפורמט הבא:
    ...
    **שאלה: [שאלה שאנשים שואלים בגוגל על נושא הדף]**
    תשובה: [תשובה ספציפית, ישירה, 2-3 משפטים — ניתנת לציטוט ישיר על ידי AI]
    (חזור על הפורמט לכל 3 שאלות)
    חשיבות קריטית ל-GEO: Gemini, ChatGPT ו-Perplexity מחלצים ומצטטים Q&A ישירות בתשובותיהם.
...
6. FAQ_SCHEMA: בסוף הפלט, העתק את כל שאלות ה-FAQ מהפוסט בפורמט FAQ_SCHEMA. כל תשובה: 40-60 מילים.
...
Q: [שאלה 3]
A: [תשובה]
[המשך לכל שאלות ה-FAQ בפוסט — לפחות 4, מקסימום 8]
"""
...
def build_image_prompt(topic, config=None, site_context=None):
    ...
    if image_style:
        ...
        hex_primary = image_style.get("brand_hex_primary", "")
        hex_secondary = image_style.get("brand_hex_secondary", "")
        visual_style = image_style.get("visual_style", "high quality stock photography, real people or objects, no graphics")

        if hex_primary and hex_secondary:
            color_str = (
                f"Color grading: dominant {hex_primary} deep blue tones, "
                f"{hex_secondary} warm accent highlights, clean white backgrounds. "
                f"Visual consistency: all images from this brand should look like they belong in the same catalogue. "
            )
        else:
            color_str = f"{color_palette} " if color_palette else ""
...
def build_cluster_linking_prompt(post_data, cluster_info, config):
    """
    Build a prompt to update a post's internal links for topical cluster coherence.
    Keeps all content intact — only adds/updates internal links to cluster members.

    post_data:    existing post dict (title, body/content_text)
    cluster_info: {"is_pillar": bool, "pillar": post_dict, "satellites": [post_dict, ...]}
    ...
    is_pillar = cluster_info.get("is_pillar", False)

    if is_pillar:
        link_targets = cluster_info.get("satellites", [])
        role_instruction = "פוסט זה הוא הפוסט הראשי (Pillar) בנושא. הוא צריך לקשר לכל הפוסטים הלוויינים."
    else:
        pillar = cluster_info.get("pillar", {})
        link_targets = [pillar] if pillar else []
        role_instruction = "פוסט זה הוא פוסט לוויין. הוא צריך לקשר לפוסט הראשי."

    links_list = "\n".join(
        f"- [{p['title']}]({blog_url}/{p['title'].replace(' ', '-')})"
        for p in link_targets
        if p.get("title")
    )

    return f"""שמור על התוכן המלא של הפוסט — שנה רק קישורים פנימיים.

פוסט: {post_title}
{role_instruction}

קישורים שחייבים להופיע בפוסט (הוסף בצורה טבעית לתוך הטקסט הקיים):
{links_list}

החזר את הפוסט המלא עם הקישורים המוטמעים. שמור על TITLE/META_DESCRIPTION/SLUG המקוריים.
שפה: עברית.

=== פורמט פלט ===
TITLE: {post_title}
META_DESCRIPTION: [תיאור מטא קיים — שמור ללא שינוי]
SLUG: [slug קיים — אל תשנה]

---

[תוכן הפוסט המלא עם הקישורים המוטמעים]
